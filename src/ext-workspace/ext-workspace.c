...
workspace_handle_activate(struct wl_client *client, struct wl_resource *resource)
{
	...
	dwl_transaction_op_add(addon->ctx, WS_PENDING_WS_ACTIVATE,
		workspace, /*data*/ NULL);
}
...
workspace_handle_deactivate(struct wl_client *client, struct wl_resource *resource)
{
	...
	struct dwl_ext_workspace *workspace = addon->data;
	dwl_transaction_op_add(addon->ctx, WS_PENDING_WS_DEACTIVATE,
		workspace, /*data*/ NULL);
}
		...
		struct wl_resource *new_group_resource)
{
	...
	dwl_transaction_op_add(addon->ctx, WS_PENDING_WS_ASSIGN, workspace, new_grp);
}
...
workspace_handle_remove(struct wl_client *client, struct wl_resource *resource)
{
	...
	struct dwl_ext_workspace *workspace = addon->data;
	dwl_transaction_op_add(addon->ctx, WS_PENDING_WS_REMOVE,
		workspace, /*data*/ NULL);
}
...
static void
workspace_send_state(struct dwl_ext_workspace *workspace, struct wl_resource *target)
{
	if (target) {
...
static void
workspace_send_initial_state(struct dwl_ext_workspace *workspace, struct wl_resource *resource)
{
...
static void
workspace_set_state(struct dwl_ext_workspace *workspace,
		...
		struct wl_resource *resource, const char *name)
{
	...
	struct dwl_transaction_op *transaction_op = dwl_transaction_op_add(
...
group_resource_create(struct dwl_ext_workspace_group *group,
		struct wl_resource *manager_resource, struct dwl_transaction_session_context *ctx)
{
	struct wl_client *client = wl_resource_get_client(manager_resource);
	struct wl_resource *resource = wl_resource_create(client,
...
static void
group_send_state(struct dwl_ext_workspace_group *group, struct wl_resource *resource)
{
...
/* Internal API */
void
ext_manager_schedule_done_event(struct dwl_ext_workspace_manager *manager)
{
	if (manager->idle_source) {
		return;
	}
...
static void
send_group_workspace_event(struct dwl_ext_workspace_group *group,
		struct dwl_ext_workspace *workspace,
...
dwl_ext_workspace_manager_create(struct wl_display *display, uint32_t caps, uint32_t version)
{
	assert(display);
...
void
dwl_ext_workspace_group_destroy(struct dwl_ext_workspace_group *group)
{
	assert(group);
...
dwl_ext_workspace_create(struct dwl_ext_workspace_manager *manager, const char *id)
{
	assert(manager);
...
void
dwl_ext_workspace_assign_to_group(struct dwl_ext_workspace *workspace,
		struct dwl_ext_workspace_group *group)
{
	assert(workspace);
...
void
dwl_ext_workspace_set_name(struct dwl_ext_workspace *workspace, const char *name)
{
	assert(workspace);
	assert(name);
...
void
dwl_ext_workspace_set_active(struct dwl_ext_workspace *workspace, bool enabled)
{
	assert(workspace);
...
void
dwl_ext_workspace_set_urgent(struct dwl_ext_workspace *workspace, bool enabled)
{
	assert(workspace);
...
void
dwl_ext_workspace_set_hidden(struct dwl_ext_workspace *workspace, bool enabled)
{
	assert(workspace);
...
void
dwl_ext_workspace_set_coordinates(struct dwl_ext_workspace *workspace,
...
void
dwl_ext_workspace_destroy(struct dwl_ext_workspace *workspace)
{
	assert(workspace);
	...
	wl_resource_for_each_safe(ws_res, ws_tmp, &workspace->resources) {
		...
		struct dwl_wl_resource_addon *ws_addon = wl_resource_get_user_data(ws_res);
